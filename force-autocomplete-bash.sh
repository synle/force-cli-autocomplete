_force_cli_completion()
{
    ...
    local ALL_TYPES="ActionLinkGroupTemplate AnalyticSnapshot CustomApplication AppMenu ApprovalProcess AssignmentRules AuthProvider AuraDefinitionBundle AutoResponseRules CallCenter PlatformCachePartition Certificate ChannelLayout ApexClass Community ApexComponent ConnectedApp CorsWhitelistOrigin CustomApplicationComponent CustomMetadata CustomPermission Dashboard ExternalDataSource DataCategoryGroup DelegateGroup Document EmbeddedServiceConfig EmailTemplate EscalationRules CustomFeedFilter FlexiPage FlowDefinition Flow GlobalPicklist Group HomePageComponent HomePageLayout InstalledPackage CustomLabels Layout LeadConvertSettings Letterhead MatchingRules NamedCredential CustomObject CustomObjectTranslation ApexPage PathAssistant PermissionSet PostTemplate Profile PostTemplate PostTemplate Profile Queue QuickAction RemoteSiteSetting Report ReportType Role Scontrol Settings SharingRules SiteDotCom CustomSite StaticResource SynonymDictionary CustomTab ApexTrigger CustomPageWebLink Workflow CspTrustedSite"
